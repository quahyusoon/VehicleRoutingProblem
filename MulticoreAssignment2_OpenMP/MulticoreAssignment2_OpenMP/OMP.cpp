...
FILE *pfile[100];
int cities=0,amountCombi=0,car;
...
/*===================writeFiles===================
			write the shortest path into separate files which 
			created in threads
*/
...
int main()
{
	...
	int no_of_point,x_value,y_value,a;
	int permu_src[30];
	int combi_arr[10];
	char fileNo[2];
	char filename[20];
	...
	if (cities-1>car)
	{
		...
		{	
				#pragma omp for schedule(dynamic,1) 
				for(int fileNum=0;fileNum<car;fileNum++)
				{
					itoa(fileNum,fileNo,10);
					strcpy(filename,"Solution");
					strncat(filename,fileNo,10);
					strcat(filename,".vrp");
					pfile[fileNum]=fopen(filename,"w");
				}
		}
